...
export const addproduct = async (req, res) => {
    const { title, flavor, description, category, urlImage } = req.body;
    try {
        if (!title || !flavor || !description || !category || !urlImage) {
            return res.status(401).json({
                message: "Something is missing ,please check",
                success: false
            })
        ...
        await Product.create({ title, flavor, description, category, urlImage })
        let product = ({ title, flavor, description, category, urlImage })
        return res.status(200).json({
...
export const listProductCategory = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({
            message: "fetched category product",
            success: true,
